import {StaticImageData} from "next/image";

import Networking from "../../assets/image/portfolio/network.png";
import Code from "../../assets/image/portfolio/code.png";
import Wordpress from "../../assets/image/portfolio/wordpress.png";
import DryasLibrary from "../../assets/image/portfolio/dryaslibrary.png";
import Corona from "../../assets/image/portfolio/corona.png";
import SportsSkuyy from "../../assets/image/portfolio/sportsskuyy.png";
import StackGames from "../../assets/image/portfolio/stack-games.png";
import OurEnterprise from "../../assets/image/portfolio/ourenterprise.png";
import Veyaz from "../../assets/image/portfolio/veyaz.png";
import KSPSumberRejeki from "../../assets/image/portfolio/ksp-sumberrejeki.png";
import Andromart from "../../assets/image/portfolio/andromart.png";
import BatuBeling from "../../assets/image/portfolio/batubeling.png";
import ModernCocoInternational from "../../assets/image/portfolio/mci.png";
import PolaPedia from "../../assets/image/portfolio/polapedia.png";
import IKAPENS from "../../assets/image/portfolio/ikapens.png";
import HMM from "../../assets/image/portfolio/hmm.png";
import FirstMedia from "../../assets/image/portfolio/first-media.png";
import FarmerDistribution from "../../assets/image/portfolio/farmer-distribution.png";
import CSRCleaning from "../../assets/image/portfolio/csr-cleaning.png";
import AsiaCommerce from "../../assets/image/portfolio/asiacommerce.png";
import WahyuDewanagari from "../../assets/image/portfolio/wahyu-dewanagari.png";
import Kubota from "../../assets/image/portfolio/kubota.png";
import Sakpattana from "../../assets/image/portfolio/sakpattana.png";

export const imageAssets: Record<string, StaticImageData> = {
    Networking,
    Code,
    Wordpress,
    DryasLibrary,
    Corona,
    SportsSkuyy,
    StackGames,
    OurEnterprise,
    Veyaz,
    KSPSumberRejeki,
    Andromart,
    BatuBeling,
    ModernCocoInternational,
    PolaPedia,
    IKAPENS,
    HMM,
    FirstMedia,
    FarmerDistribution,
    CSRCleaning,
    AsiaCommerce,
    WahyuDewanagari,
    Kubota,
    Sakpattana,
};
